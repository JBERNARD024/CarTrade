...
class EditCar: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnAdicionarCarro.setOnClickListener {
            ...
            if(verificaCampos()){
                val id = cars.size+1
                val car = Car(id,marca, modelo, versao, combustivel, ano, kms, potencia, cilindrada, cor, tipoCaixa, numMudancas, numPortas, lotacao, condicao, preco, "")
                updateCar(car) {
                    Toast.makeText(this,"Update bem sucedido", Toast.LENGTH_SHORT).show()
                }
            }else{
    ...
    private fun updateCar(car: Car, onResult: (APIResult?) -> Unit){
        val call = Retrofit().getService().updateCar(car.id, car.marca, car.modelo, car.versao, car.combustivel,
            car.ano, car.kms, car.potencia, car.cilindrada, car.cor, car.tipoCaixa, car.numMudancas,
            car.numPortas, car.lotacao, car.condicao, car.preco)
        call.enqueue(
